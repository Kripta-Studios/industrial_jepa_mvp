...
Table~\ref{tab:visual} shows the current quick-run result. DINOv2 was actually loaded from local \texttt{torch.hub} cache; this is not a random fallback. The result is deliberately scoped: one category, quick configuration, not a multi-category production validation.

\begin{table}[t]
\centering
\caption{Visual foundation quick runs on MVTec AD bottle.}
\label{tab:visual}
\footnotesize
...
\begin{table}[t]
\centering
\caption{Consolidated CNC sensor evidence from the previous phase.}
\label{tab:sensor}
\footnotesize
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.12}
