...
Held-out-hardness values in this manuscript are invalidated: equal raw physical
hardness values could be assigned to different bins/splits. Visual thresholded
metrics that selected a threshold on test are also invalid. No post-fix metric
is inserted here; tables remain historical until a reproducible rerun.}}
\end{center}
...
\begin{itemize}
    ...
    \item \textbf{Hard generalization protocols}; material-hardness rows require a post-fix rerun with raw values disjoint.
\end{itemize}
...
Table~\ref{tab:hardgen} is a historical pre-fix record. Its held-out-hardness rows do not establish unseen physical-value generalization and must not be compared with post-fix results.

\begin{table*}[t]
\centering
\caption{Historical hard-generalization AUPRC; hardness rows invalidated pending rerun.}
\label{tab:hardgen}
\footnotesize
\setlength{\tabcolsep}{4pt}
...
\begin{table*}[t]
\centering
\caption{Invalidated pre-fix JEPA vs. engineered held-out-hardness split.}
